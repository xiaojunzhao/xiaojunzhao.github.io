...
<p style='text-align: justify;'>The forget gate \(f(t)\) is capable of deciding which information to forget from the previous cell state \(C(t-1)\), because the output of the sigmoid function \(\sigmoid\) is a single value between 0 and 1. Multiplying this value with an element of \(C(t-1)\) means how much information we're going to throw away, or put in another way, how much informatio we're going to keep.  Here, we notice that we concate the previous hidden state \(h_{t-1}\) with the input vector \(X_{t}\) together. </p>
